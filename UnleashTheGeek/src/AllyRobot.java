            ...
            if (destination != null) {
                ...
                if (unknownCells.size() > 10) {
                    ...
                    if (radarsToDetroy.size() > 0) {
                        Radar closestRadarToDestroy = radarsToDetroy.parallelStream().min((c1, c2) -> java.lang.Double.compare(c1.distance(this), c2.distance(this))).orElse(null);
        ...
        if (carry == RobotCarry.ORE) {
            ...
            if (destination != null && world.getCellAt(destination).getNbOre() > 0) {
                ...
                if (!cellsWithOre.isEmpty()) {
                    Cell possibleDestination = cellsWithOre.parallelStream().min((c1, c2) -> java.lang.Double.compare(c1.distance(this), c2.distance(this))).orElse(null);
